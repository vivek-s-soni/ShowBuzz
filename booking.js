...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!localStorage.getItem("showbuzz_content")) {

        const contentData = [
            { name: "Dhurandhar", type: "movie" },
            { name: "Chhavva", type: "movie" },
            { name: "Border 2", type: "movie" },
            { name: "F1", type: "movie" },
            { name: "WAR-2", type: "movie" },

            { name: "Arijit Singh", type: "event" },
            { name: "Aditya Gadhvi", type: "event" },
            { name: "ICC Mens Cricket World Cup", type: "event" },
            { name: "AI-Summit", type: "event" },
            { name: "Coldplay-2026", type: "event" },

            { name: "Anubhav Singh Bassi", type: "comedy" },
            { name: "Harsh Gujral", type: "comedy" },
            { name: "Zakir Khan", type: "comedy" },
            { name: "Vipul", type: "comedy" },
            { name: "Pranit More", type: "comedy" }
        ];

        localStorage.setItem("showbuzz_content", JSON.stringify(contentData));
    }
...
function proceedToPayment() {
    ...
    // Store current payable amount
    window.paymentTotalAmount = totalAmount;

    // Get wallet balance
    const currentUser = localStorage.getItem('showbuzz_currentUser');
    const users = getUsers();
    const walletBalance = users[currentUser]?.wallet || 0;

    // Display wallet balance
    document.getElementById("walletBalanceDisplay").innerText = walletBalance;
...
function updateProfile() {
    const oldEmail = localStorage.getItem('showbuzz_currentUser');
    let users = getUsers();
    ...
    if (newEmail !== oldEmail) {
        ...
        // Move bookings to new email
        if (bookings[oldEmail]) {
            bookings[newEmail] = bookings[oldEmail];
            delete bookings[oldEmail];
        }

        // Update current logged user
        localStorage.setItem('showbuzz_currentUser', newEmail);

    } else {
        // If email not changed → just update other fields
        users[oldEmail].name = newName;
        users[oldEmail].mobile = newMobile;
        users[oldEmail].age = newAge;
    }

    localStorage.setItem('showbuzz_users', JSON.stringify(users));
...
function cancelBooking(index) {
    ...
    // Get refund amount
    const refundAmount = bookings[currentUser][index].totalAmount;

    // Add refund to wallet
    users[currentUser].wallet += refundAmount;

    // Remove booking
    bookings[currentUser].splice(index, 1);

    // Save updates
    localStorage.setItem('showbuzz_users', JSON.stringify(users));
    localStorage.setItem('showbuzz_bookings', JSON.stringify(bookings));

    alert(`✅ Booking cancelled!\n💰 ₹${refundAmount} refunded to your ShowBuzz Wallet.`);
...
function deleteAccount() {
    ...
    delete bookings[currentUser];

    localStorage.setItem('showbuzz_users', JSON.stringify(users));
    localStorage.setItem('showbuzz_bookings', JSON.stringify(bookings));
    localStorage.removeItem('showbuzz_currentUser');
...
function useWalletBalance() {
    ...
    let walletBalance = users[currentUser].wallet || 0;
    let totalAmount = window.paymentTotalAmount;
    ...
    if (walletBalance >= totalAmount) {
        ...
        users[currentUser].wallet -= totalAmount;
        window.paymentTotalAmount = 0;

        localStorage.setItem('showbuzz_users', JSON.stringify(users));

        alert("✅ Full payment done using wallet!");
        ...
        generateBookingSummary(totalAmount, "Wallet");

    } else {
        // Partial wallet payment
        window.paymentTotalAmount = totalAmount - walletBalance;

        users[currentUser].wallet = 0;

        localStorage.setItem('showbuzz_users', JSON.stringify(users));

        document.getElementById("paymentTotalAmount").innerText = window.paymentTotalAmount;

        alert(`💰 ₹${walletBalance} used from wallet.\nRemaining amount: ₹${window.paymentTotalAmount}`);
    }

    document.getElementById("walletBalanceDisplay").innerText = users[currentUser].wallet;
}
